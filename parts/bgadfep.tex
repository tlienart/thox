...
%
We can therefore reinterpret the algorithm more generally as targeting a distribution $p$ that factorises in a fixed number (say $K$) of factors $p(x)\propto \pi_{0}(x)\prod_{i=1}^{K}t_{i}(x)$.\add{notations: make sure it's clear that $\pi_{0}$ is always the prior} this is the Assumed Density Filtering algorithm. Note that in this case there is no ``natural ordering'' as in the online learning case and hence the algorithm can be done for any permutation of the factors.
%
...
%In the Assumed Density Filtering algorithm, the approach is very similar except that we just consider that the target density factorises into $K$ terms: $p(x)\propto\prod_{i=1:K}t_{i}(x)$ 

\todofr{ok good, now explain how this can be recycled for a fixed number of terms but then how the question of ordering arises which leads to EP}

%\add{Add some history stuff suggesting there's multiple stuff called ADF, cite stuff following Minka}
%In the context of online Bayesian learning, one is interested in approximating the posterior given a 
