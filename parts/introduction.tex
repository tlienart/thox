...
We also illustrate one specific type of loopy graph: the \emph{grid graph}.\check{sep14, jun22}
...
\eqa{	\syst{
	...
	\nn}
where $\mu_0$ as well as the $Q_i$, $R_i$, $A_i$, $a_i$, $B_i$ and the $b_i$ are fixed and deterministic. In such a case, an analytical expression for both the filtering and the smoothing densities can be obtained through the well-known \emph{Kalman filter} and \emph{Kalman smoother} \citep{anderson79}. \check{sep14}
...
In image processing, grid graphs can be used to model interaction between the pixels of an image  \citep{blake11}. 
