...
%
where $p_0$ is a prior distribution. Each of the likelihood term itself factors \add{COMPLETE, go from this to general factorised form, assume that the $t_i$ are positive}
...
%
The first part, denoted by a $\mathcal E$ is the EP energy.\add{add footnote explaining that sometimes defined with different sign but that it doesn't matter} Note that its gradient in $\lambda_i$ is simply $\nabla_{\lambda_i}\mathcal E = (-\nabla A(\theta) + \E_{q_i}[\phi])$. 
