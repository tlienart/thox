...
An issue with the SMS algorithm which we will expose in more details in the experiments\add{do it} is that it performs the projection of the noisy estimator $\hat\mu_{i}$ before damping it. Since this projection operator is likely non-linear and potentially numerically unstable\add{clarify} this can hinder the performances of the algorithm. It seems therefore sensible to damp the estimator before it gets projected. This leads to going from \eqref{eq:damped-update-np} to
%
\eqa{
...
%
Anyone familiar with convex optimisation will notice the similarity with the \emph{mirror-descent algorithm} \citep{nemirovski83, beck03}. We will explore and discuss this similarity later.\add{do it} 
