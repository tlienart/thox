...
The problem of computing the singleton marginal on a tree is simpler since the recursive structure of the problem can easily be exploited. 
This leads to the BP algorithm which, on a tree, leads to functions (the beliefs) that are proportional to the true marginals.

Let $s$ denote the node of interest so that we are targeting the marginal $p(x_{s})$. 
Then, the full joint on the tree can be written relatively to that node as:
...
\eqa{	\phi_{\partial t\backslash s}(x_t,x_{\partial t\backslash s}) &=& \prod_{u\in\partial t\backslash s} \psi_{ut}(x_u,x_t)\pac{\psi_u(x_u)\phi_{\partial u\backslash t}(x_u,x_{\partial u\backslash t})},	\nn}
and $\phi_\emptyset \equiv 1$ (for a leaf node). 
This decomposition is illustrated at the figure \ref{BP for tree 1}.
%%
\begin{figure}[!h]
\center
...
%\begin{figure}[!h]
%\center
%\includegraphics[scale=1]{_figs/treeFact}
%\caption{\label{BP for tree 1} Illustration of the elements of the factorisation: the node potential $\psi_{s}$ of the node of interest, the potential corresponding to the edge connecting it to one of its neighbour $t$: $\psi_{st}$, the potential at that node, and the product of all subsequent potentials attached to that node $\phi_{\partial t\backslash s}$.}
%\end{figure}

Using this recursive structure, and integrating out all variables apart from $x_{s}$, the marginals can be written as
\eqa{		p_{s}(x_{s}) &\propto& \psi_{s}(x_{s})\prod_{t\in\partial s} \int \psi_{ts}(x_{t},x_{s}) \pac{\psi_{t}(x_{t})\underbrace{\int \phi_{\partial t\backslash s}(x_{t},x_{\partial t\backslash s}) \dx_{\partial t\backslash s}}_\kappa}\dx_{t}.\nn	}
Now, exploiting the recursive structure of $\phi_{\partial t\backslash s}$, we can expand the $\kappa$ term:
\eqa{	\kappa &=& \prod_{u\in \partial t\backslash s} \int \psi_{ut}(x_u,x_t)\pac{\psi_u(x_u)\int\phi_{\partial u\backslash t}(x_u,x_{\partial u\backslash t})\mathrm{d}x_{\partial u\backslash t}}\mathrm{d}u.\nn}
...
\eqa{	M_{ts}(x_t) &:=& \psi_t(x_t) \prod_{u\in\partial t\backslash s}m_{u t}(x_t),	}
...
\eqa{		\syst{	M_{r s}(x_{r}) &=& \psi_{r}(x_{r}) \prod_{u\in\partial r\backslash s} m_{u r}(x_{r}) \\
				m_{s  r}(x_{r}) &=& \int\psi_{sr}(x_{s},x_{r})M_{s r}(x_{s})\dx_{s}\\
				B_{r}(x_{r}) &=& m_{s r}(x_{r})M_{r\to s}(x_{r})	}.	\label{eq structure bp}}
...
Propagating messages through different ordering of the nodes can therefore lead to inconsistent messages. \add{show?}
...
%
\eqa{		\syst{	M_{r s}^{n}(x_{r}) &=& \psi_{r}(x_{r}) \prod_{u\in\partial r\backslash s} m^{n-1}_{u r}(x_{r}) \\
				m^{n}_{s r}(x_{r}) &=& \int \psi_{sr}(x_{s},x_{r})M^{n}_{s r}(x_{s})\dx_{s}\\
				B^{n}_{r}(x_{r}) &=& m^{n}_{s r}(x_{r})M^{n}_{r s}(x_{r})	}.	\label{eq structure lbp}
				}
%
